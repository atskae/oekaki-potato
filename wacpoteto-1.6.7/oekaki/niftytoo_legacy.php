...
This file is a duplicate of niftytoo.php, except with anonymous functions to replace the obsolete preg 'e' modifier.  Do not attempt to use this code with PHP7+.
*/
...
function nifty2_convert($out) {
	...
	if (strpos($out, '://') !== false) {
		// Legacy mode for PHP < 5.3.0
		$out = preg_replace('!([^]:,=]|^)(https?://[[:alnum:]-+\(\)\[\]&@#/%\?\$=~_\'|:,.;]+[[:alnum:]-+&@#/%=~_|;])!i'
			, "\\1<a href=\"\\2\">\\2</a>"
			, $out);
	}
...
function nifty2_replace(&$text, $tag, $replace, $flags = 's', $parameter = null, $filter_fn = null) {
	...
	if ($filter_fn) {
		// Rewrite string as a PHP regex eval.  Note use of ${1}
		// replacement form, as PHP does not handle backspaces like Perl.

		$replace = str_replace("'", "\\'", $replace);
		$replace = "'".preg_replace('!(\%f)([0-9])!', "'.".$filter_fn."('\\\${\\2}').'", $replace)."'";
		$flags .= 'e';
	}
