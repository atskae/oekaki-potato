...
if ($mode == 'norm' || $mode == 'anim') {
	...
	if ($resno == 0) {
		w_log(WLOG_FAIL, 'paintbbsget: $l_no_picid');
	...
	// Insert new slot
	$sqlres = "INSERT INTO {$db_p}oekakidta SET usrname='$OekakiU', postdate=NOW(), hostname='$hostname', comment='', PIC_ID='$resno', IP='$address', datatype=0, edittime='$thetime', password=''";
	if ($mode == 'anim') {
		$sqlres .= ', animation=1';
	} else {
		$sqlres .= ', animation=0';
	...
	if (!$result) {
		w_log_sql('paintbbsget: $l_app_no_insert`'.$resno.'`');
...
if (!save_image($image['o_data'], $resno)) {
	w_log(WLOG_FAIL, 'paintbbsget: $l_app_no_save`'.$resno.'`');
	quiet_exit( t('err_saveimg'));
}
