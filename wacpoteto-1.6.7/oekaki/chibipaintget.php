...
Chibi Paint sends data as multipart encoded (yay!), so this code is quite different from the other oekaki applets.

Chibi Paint does not support animation data, but the layers file is still useful so we'll save it as an animation, anyway.
*/
...
if ($mode == 'norm' || $mode == 'anim') {
	...
	if ($resno == 0) {
		w_log(WLOG_FAIL, 'chibipaintget: $l_no_picid');
	...
	// Insert new slot
	$sqlres = "INSERT INTO {$db_p}oekakidta SET usrname='$OekakiU', postdate=NOW(), hostname='$hostname', comment='', PIC_ID='$resno', IP='$address', datatype=4, edittime='$thetime', password=''";
	...
	if (!$result) {
		w_log_sql('chibipaintget: $l_app_no_insert`'.$resno.'`');
...
if (!save_image($buffer, $resno)) {
	w_log(WLOG_FAIL, 'chibipaintget: $l_app_no_save`'.$resno.'`');
	quiet_exit( t('err_saveimg'));
}
